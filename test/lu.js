...
contract("Lockup", function(accounts) {
  it("check", async function() {
    ...
    assert.equal(
      await lockup.isTransferAllowed.call(
        accounts[1],
        new BigNumber("100").mul(precision).valueOf()
      ),
      true,
    ...
    assert.equal(
      await lockup.isTransferAllowed.call(
        accounts[1],
        new BigNumber("90").mul(precision).valueOf()
      ),
      true,
      "isTransferAllowed is not equal"
    );
    ...
    await ico.changeICODates(3, since, since + 3600 * 24);
    ...
    await lockup.logLargeContribution(
      accounts[1],
    ...
    await ico.changeICODates(3, since - 15778476 - 3600, since - 15778476);
