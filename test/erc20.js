var ERC20 = artifacts.require("./OpsERC20.sol");
...
contract("ERC20", function(accounts) {
  ...
  it("deploy & check for total supply & balance of smart contract & sender", function() {
    ...
    return ERC20.new(1000000, "TEST", 18, "TEST", false, true)
      .then(function(_instance) {
        instance = _instance;
      })
      .then(() =>
        Utils.balanceShouldEqualTo(instance, instance.address, 1000000)
      )
  ...
  it("transfer with enabled lock", function() {
    ...
    return ERC20.new(1000000, "TEST", 18, "TEST", true, true)
      .then(function(_instance) {
        instance = _instance;
      })
  ...
  it("transfer with disabled lock", function() {
    ...
    return ERC20.new(1000000, "TEST", 18, "TEST", true, false)
      .then(function(_instance) {
        instance = _instance;
      })
      .then(() => Utils.balanceShouldEqualTo(instance, accounts[0], 1000000))
  ...
  it("approve, transfer by transferFrom", function() {
    ...
    return ERC20.new(1000000, "TEST", 18, "TEST", true, false)
      ...
      .then(() => Utils.balanceShouldEqualTo(instance, accounts[1], 0))
      .then(function() {
        return instance.transferFrom(accounts[0], accounts[1], 1001, {
          from: accounts[1],
        });
      })
      .then(Utils.receiptShouldFailed)
  ...
  it("approve, transferFrom more than exists", function() {
    ...
    return ERC20.new(1000000, "TEST", 18, "TEST", true, false)
      ...
      .then(() => Utils.balanceShouldEqualTo(instance, accounts[1], 0))
      .then(function() {
        return instance.transferFrom(accounts[0], accounts[1], 1000001, {
          from: accounts[1],
        });
      })
      .then(Utils.receiptShouldFailed)
  ...
  it("shouldn't not be able increase or decrease when contract is locked then unlock", async function() {
    ...
    let postIncrease;
    await instance
      .increaseApproval(accounts[1], 50)
    ...
    await instance
      .decreaseApproval(accounts[1], 10)
      .then(() => instance.allowance.call(accounts[0], accounts[1]))
  ...
  it("should increase by 50 then decrease by 10", async function() {
    let postIncrease = 0,
      preApproved = 0;
    token = await ERC20.new(1000000, "TEST", 18, "TEST", true, false);
    await token
      .increaseApproval(accounts[1], 50)
      // preApproved.plus(50).should.be.bignumber.equal(postIncrease);
    ...
    await token
      ...
      .then((result) =>
        assert.equal(
          result.valueOf(),
          postIncrease - 10,
          "should be  postIncrease.minus(10)"
        )
      );
  ...
  it("shouldn't not be able increase or decrease when contract is locked", async function() {
    let postIncrease = 0,
      preApproved = 0;
    token = await ERC20.new(1000000, "TEST", 18, "TEST", true, true);
    await token
      .increaseApproval(accounts[1], 50)
      .then(() => token.allowance.call(accounts[0], accounts[1]))
      .then((result) => {
    ...
    await token
      ...
      .then(() => token.allowance.call(accounts[0], accounts[1]))
      .then((result) =>
        assert.equal(result.valueOf(), 0, "should be  postIncrease.minus(10)")
      );
  });
});
