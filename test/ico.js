...
contract("ICO", function(accounts) {
  it("deploy & check constructor info & setPrivateSale & getStats", async function() {
    ...
    assert.equal(
      ...
      "soldTokens is not equal"
    );
    assert.equal(
      preICOStats[1],
      new BigNumber("0").valueOf(),
      "collectedUSD is not equal"
    );
    assert.equal(
      preICOStats[2],
      new BigNumber("0").valueOf(),
      "collectedEthers is not equal"
    );
    assert.equal(preICOStats[3], false, "burned is not equal");

    await ico
      .setPrivateSale(privateico.address, { from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await ico
      .setPrivateSale(privateico.address)
      .then(Utils.receiptShouldSucceed);

    let stats = await ico.getStats.call();
    assert.equal(
      stats[0],
      new BigNumber(1526256000).valueOf(),
      "startTime is not equal"
    );
    assert.equal(
      stats[1],
      new BigNumber(1527984000).valueOf(),
      "endTime is not equal"
    );
    assert.equal(
      stats[9][0],
      new BigNumber("40000000").mul(precision).valueOf(),
      "maxAmount is not equal"
    );
    assert.equal(
      stats[9][1],
      new BigNumber("0.06").mul(usdPrecision).valueOf(),
      "price is not equal"
    );
    assert.equal(
      stats[9][2],
      new BigNumber(1525046400).valueOf(),
      "startTime is not equal"
    );
    assert.equal(
      stats[9][3],
      new BigNumber(1525564800).valueOf(),
      "endTime is not equal"
    );
    assert.equal(
      stats[9][4],
      new BigNumber("150000000").mul(precision).valueOf(),
      "maxAmount is not equal"
    );
    assert.equal(
      stats[9][5],
      new BigNumber("0.08").mul(usdPrecision).valueOf(),
      "price is not equal"
    );
    assert.equal(
      stats[9][6],
    ...
    assert.equal(
      stats[9][7],
      new BigNumber(1526774400).valueOf(),
      "endTime is not equal"
    );
    assert.equal(
      stats[9][8],
    ...
    assert.equal(
      stats[9][9],
      new BigNumber("0.10").mul(usdPrecision).valueOf(),
      "price is not equal"
    );
    assert.equal(
      stats[9][10],
      new BigNumber(1526860800).valueOf(),
      "startTime is not equal"
    );
    assert.equal(
      stats[9][11],
      new BigNumber(1527379200).valueOf(),
      "endTime is not equal"
    );
    assert.equal(
      stats[9][12],
    ...
    assert.equal(
      stats[9][13],
      new BigNumber("0.12").mul(usdPrecision).valueOf(),
      "price is not equal"
    );
    assert.equal(
      stats[9][14],
      new BigNumber(1527465600).valueOf(),
      "startTime is not equal"
    );
    assert.equal(
      stats[9][15],
  ...
  it("calculateTokensAmount ", async function() {
    ...
    let tokensAmount = await ico.calculateTokensAmount(
    ...
    tokensAmount = await ico.calculateTokensAmount(
    ...
    tokensAmount = await ico.calculateTokensAmount(
      new BigNumber(minInvest).valueOf(),
      true
    );
    assert.equal(
      new BigNumber(tokensAmount[0]).valueOf(),
    ...
    tokensAmount = await ico.calculateTokensAmount(
      new BigNumber(minInvest).sub(1).valueOf(),
      true
    );
  ...
  it("check buy & transfer ethers", async function() {
    ...
    let compensationAddressBalance = await Utils.getEtherBalance(
      compensationAddress
    ).valueOf();
      ...
      { ico, token },
      {
        token: {
          totalSupply: new BigNumber("10000000")
            .add("12500")
            .add("16666.666666666666666666")
            .add("62.5")
            .mul(precision)
            .valueOf(),
          balanceOf: [
            {
              [accounts[3]]: new BigNumber("12500")
                .add("16666.666666666666666666")
                .mul(precision)
                .valueOf(),
            },
            {
              [compensationAddress]: new BigNumber("62.5")
                .mul(precision)
                .valueOf(),
            },
          ],
    ...
    await ico.testChangeSoftCap(
      new BigNumber("3000").mul(usdPrecision).valueOf()
    );
    await ico.testChangeCompensationAmount(
      new BigNumber("2000").mul(usdPrecision).valueOf()
    );

    await ico
      .sendTransaction({
        value: new BigNumber("2").mul(precision).valueOf(),
        from: accounts[3],
      })
      .then(Utils.receiptShouldSucceed);
    ...
    console.log(
      "compensation address balance before",
      new BigNumber(compensationAddressBalance).valueOf()
    );
    console.log(
      "compensation address balance afterr",
      new BigNumber(
        await Utils.getEtherBalance(compensationAddress).valueOf()
      ).valueOf()
    );
    ...
    console.log(
      "---------",
      new BigNumber(
        await Utils.getEtherBalance(compensationAddress)
          .sub(compensationAddressBalance)
          .valueOf()
      ).valueOf()
    );

    console.log(
      "etherHolder address balance before",
      new BigNumber(ethHolderBalance).valueOf()
    );
    console.log(
      "etherHolder address balance afterr",
      new BigNumber(
        await Utils.getEtherBalance(etherHolder).valueOf()
      ).valueOf()
    );
    console.log("should be", new BigNumber("2").mul(precision).valueOf());
    console.log(
      "---------",
      new BigNumber(
        await Utils.getEtherBalance(etherHolder)
  ...
  it("check refund", async function() {
    ...
    await token.setICO(ico.address).then(Utils.receiptShouldSucceed);
    await token
      .setPrivateSale(privateico.address)
      .then(Utils.receiptShouldSucceed);
    ...
    await ico
      .sendTransaction({ value: new BigNumber("2").mul(precision).valueOf() })
      .then(Utils.receiptShouldSucceed);
    ...
    assert.equal(
      await ico.refund.call({ from: accounts[2] }),
      false,
      "refund is not equal"
    );
    assert.equal(
    ...
    console.log(
      "acc0 balance before transactions -",
      new BigNumber(acc0Balance).valueOf()
    );
    console.log(
      "              after transactions -",
      new BigNumber(acc0Balance1).valueOf()
    );
    console.log(
      "              after refunddddddd -",
      new BigNumber(await Utils.getEtherBalance(accounts[0])).valueOf()
    );
    console.log(
      "                     differencee -",
      new BigNumber(new BigNumber(await Utils.getEtherBalance(accounts[0])))
        .sub(acc0Balance1)
        .div(precision)
        .valueOf()
    );
    ...
    console.log(
      "acc3 balance before transactions -",
      new BigNumber(acc3Balance).valueOf()
    );
    console.log(
      "              after transactions -",
      new BigNumber(acc3Balance1).valueOf()
    );
    console.log(
      "              after refunddddddd -",
      new BigNumber(await Utils.getEtherBalance(accounts[3])).valueOf()
    );
    console.log(
      "                     differencee -",
      new BigNumber(new BigNumber(await Utils.getEtherBalance(accounts[3])))
        .sub(acc3Balance1)
        .div(precision)
        .valueOf()
    );
  ...
  it("check burn", async function() {
    ...
    await ico
      .sendTransaction({ value: new BigNumber("1").mul(precision).valueOf() })
      .then(Utils.receiptShouldSucceed);
    ...
    await Utils.checkState(
      { ico, token },
      {
        token: {
          ...
          balanceOf: [
            {
              [accounts[3]]: new BigNumber("16666.666666666666666666")
                .add("12500")
                .mul(precision)
                .valueOf(),
            },
        ...
        ico: {
          ...
          compensationAmount: new BigNumber("51000")
            ...
            .valueOf(),
          maxTokenSupply: new BigNumber("150000000").mul(precision).valueOf(),
          soldTokens: new BigNumber("12500")
            .add("12500")
            .mul(precision)
            .valueOf(),
    ...
    await ico
      .burnUnsoldTokens({ from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
      ...
      { ico, token },
      {
        ...
        ico: {
          ...
          compensationAmount: new BigNumber("51000")
            .mul(usdPrecision)
            .valueOf(),
          maxTokenSupply: new BigNumber("12500")
            .add("12500")
            .mul(precision)
            .valueOf(),
          soldTokens: new BigNumber("12500")
