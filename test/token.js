...
contract("Token", function(accounts) {
  it("deploy & check constructor info & check: setICO setPrivateSale setLockupContract setPrivateSale, SetLocked with transfers", async function() {
    ...
    //setICO
    await token
      .setICO(accounts[2], { from: accounts[1] })
    ...
    await token
      ...
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await token.setICO(accounts[2]).then(Utils.receiptShouldSucceed);
    //setPrivateSale
    await token
      .setPrivateSale(accounts[2], { from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await token
      .setPrivateSale(0x0)
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await token.setPrivateSale(accounts[2]).then(Utils.receiptShouldSucceed);
    //setLockupContract
    await token
      .setLockupContract(accounts[2], { from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await token
      .setLockupContract(0x0)
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await token.setLockupContract(accounts[2]).then(Utils.receiptShouldSucceed);
    ...
    await token
      .transfer(accounts[1], 500)
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    await token.approve(accounts[1], 500);

    await token.transferFrom
      .call(accounts[0], accounts[1], 500, { from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);

    await token.setLocked(false).then(Utils.receiptShouldSucceed);
    assert.equal(await token.locked.call(), false, "locked is not equal");
    ...
    await token.transfer(accounts[1], 500).then(Utils.receiptShouldSucceed);
    await token.approve(accounts[1], 500);
    await token.transferFrom(accounts[0], accounts[1], 500, {
      from: accounts[1],
    });
  ...
  it("deploy & freezing with transfers", async function() {
    ...
    await token
      .freezing(false, { from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);

    assert.equal(
      await token.transferFrozen.call(),
      true,
      "transferFrozen is not equal"
    );
    ...
    await token.approve(accounts[1], 500);
    await token.transferFrom
      .call(accounts[0], accounts[1], 500, { from: accounts[0] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
    ...
    await token.setICO(ico.address).then(Utils.receiptShouldSucceed);
    await token
      .setLockupContract(lockup.address)
      .then(Utils.receiptShouldSucceed);
    assert.equal(
      await ico.isActive.call().valueOf(),
      false,
      "ico.isActive().valueOf() not equal"
    );
    await token.freezing(false);
    assert.equal(
      await token.transferFrozen.call(),
    ...
    await token.approve(accounts[1], 500);
    await token
      .transferFrom(accounts[0], accounts[1], 500, { from: accounts[1] })
      .then(Utils.receiptShouldSucceed);
  ...
  it("check burns", async function() {
    ...
    await token.setICO(accounts[4]).then(Utils.receiptShouldSucceed);
    await token.setPrivateSale(accounts[8]).then(Utils.receiptShouldSucceed);
    ...
    await token
      .burnTokens(new BigNumber(10).mul(precision).valueOf())
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);

    await token
      .burnInvestorTokens(accounts[0])
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);

    let burnedAmount = await token.burnInvestorTokens(accounts[0], {
      from: accounts[4],
    });
    assert.equal(
      new BigNumber(burnedAmount).valueOf(),
      new BigNumber("1000").valueOf(),
      "burnedAmount is not equal"
    );

    await token.burnTokens(new BigNumber("10").valueOf(), {
      from: accounts[8],
    });
