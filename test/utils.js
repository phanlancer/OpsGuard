...
function balanceShouldEqualTo(instance, address, expectedBalance, notCall) {
  return new Promise(function(resolve, reject) {
    ...
    if (notCall) {
      promise = instance.balanceOf(address).then(function() {
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      assert.equal(
        ...
        claimedAmount,
        "dividend claimed amount is not equal"
      );
      assert.equal(
      ...
      assert.equal(
        dividend.totalSupply,
        totalSupply,
        "dividend total supply is not equal"
      );
      assert.equal(
        dividend.recycleTime,
        recycleTime,
        "dividend recycle time is not equal"
      );
      assert.equal(
        dividend.recycled,
        recycled,
        "dividend recycled is not equal"
      );
...
function checkEmission(
  ...
  return new Promise(function(resolve, reject) {
    try {
      assert.equal(
        emission.blockDuration,
        blockDuration,
        "emission blockDuration is not equal"
      );
      assert.equal(
        emission.blockTokens,
        blockTokens,
        "emission blockTokens is not equal"
      );
      assert.equal(
        emission.periodEndsAt,
        periodEndsAt,
        "emission periodEndsAt is not equal"
      );
...
function checkEtherBalance(_address, expectedBalance) {
  ...
  assert.equal(
    balance.valueOf(),
    expectedBalance.valueOf(),
    "address balance is not equal"
  );
}
...
async function balanceERC20(contract, holder, balance) {
  assert.equal(
    await contract.balanceOf(holder),
    balance,
    "erc20 balance is not equal"
  );
}
...
async function testTransferFrom(contract, granter, grantee, to, tokens) {
  ...
  let trasnferFromResponse = await contract.transferFrom.call(
    granter,
    to,
    tokens + 1,
    { from: grantee }
  ...
  assert.equal(
    trasnferFromResponse.valueOf(),
    false,
    "transferFrom with exceding amount succeed"
  );
  ...
  transferFromResponse = await contract.transferFrom.call(
    granter,
    to,
    tokens / 2,
    { from: grantee }
  ...
  assert.equal(
    ...
    true,
    "transferFrom with lower amount failed"
  );
    ...
    { contract },
    {
      contract: {
        balanceOf: [
          ...
          {
            [to]: new BigNumber(granteeBeforeBalance)
              .add(new BigNumber(tokens).div(2))
              .valueOf(),
          },
        ],

        allowance: {
          __val: new BigNumber(tokens)
            .sub(new BigNumber(tokens).div(2))
            .valueOf(),
          _owner: granter,
          _spender: grantee,
...
async function checkState(contracts, states) {
  for (let contractId in states) {
    ...
    for (let stateId in states[contractId]) {
      ...
      await checkStateMethod(
        contract,
        contractId,
        stateId,
        states[contractId][stateId]
      );
    }
  }
}
