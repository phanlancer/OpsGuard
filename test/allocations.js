...
async function deploy() {
  ...
  const alloc = await Allocations.new(
    token.address,
    ico.address,
    growthAddresses
  );
...
contract("Allocations", function(accounts) {
  it("deploy & check claim", async function() {
    ...
    await alloc.setLastReleaseTime(
      parseInt(new Date().getTime() / 1000 - 3600 * 24 * 31)
    );

    await alloc.claim();

    let val = new BigNumber("100000000")
      .mul(precision)
      .div("100")
      .div("4")
      .valueOf();
      ...
      { token, ico },
      {
        token: {
          balanceOf: [
            ...
            { [accounts[3]]: new BigNumber("0").mul(precision).valueOf() },
            {
              [growthAddresses]: new BigNumber("50000000")
                .mul(precision)
                .valueOf(),
            },
          ],
