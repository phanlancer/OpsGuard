...
contract("MintinERC20", function(accounts) {
  ...
  it("should mint a given amount of tokens to a given address", async function() {
    ...
    const gasEstimate = web3.eth.estimateGas({ to: token.address, data: data });
    await token
      .mint(accounts[0], 100, { gasLimit: gasEstimate })
      .then(Utils.receiptShouldSucceed)
      .then(() =>
        Utils.balanceShouldEqualTo(
          token,
          accounts[0],
          new BigNumber("100").valueOf()
        )
      );
  ...
  it("should fail to mint for non minters", async function() {
    ...
    const gasEstimate = web3.eth.estimateGas({ to: token.address, data: data });
    await token
      .mint(accounts[0], 100, { from: accounts[1], gasLimit: gasEstimate })
  ...
  it("should be able to add minters", async function() {
    await token.addMinter(accounts[1]).then(Utils.receiptShouldSucceed);
    const data = token.contract.mint.getData(accounts[0], 100);
    const gasEstimate = web3.eth.estimateGas({ to: token.address, data: data });
    await token
      .mint(accounts[0], 100, { from: accounts[1], gasLimit: gasEstimate })
  ...
  it("should be able to remove minters", async function() {
    ...
    await token
      .mint(accounts[0], 100, { from: accounts[1] })
      .then(Utils.receiptShouldSucceed)
      .then(() =>
        Utils.balanceShouldEqualTo(
          token,
  ...
  it("should fail when non minter tries to add new minter", async function() {
    await token
      .addMinter(accounts[1], { from: accounts[1] })
      .then(Utils.receiptShouldFailed)
      .catch(Utils.catchReceiptShouldFailed);
  });
  ...
  it("should not allow to mint over max limit", async function() {
    const data = token.contract.mint.getData(
      accounts[0],
      new BigNumber(0.9).mul(precision).valueOf()
    );
    const gasEstimate = web3.eth.estimateGas({ to: token.address, data: data });
    await token
      .mint(accounts[0], new BigNumber(0.9).mul(precision), {
    ...
    await token
      .mint(accounts[0], new BigNumber(0.5).mul(precision))
      .then(Utils.receiptShouldSucceed)
      .then(() =>
        Utils.balanceShouldEqualTo(
          token,
  ...
  it("testing zero tokens minting", async function() {
    await token
      .mint(accounts[0], new BigNumber(0).mul(precision))
      .then(Utils.receiptShouldSucceed)
      .then(() =>
        Utils.balanceShouldEqualTo(
          token,
